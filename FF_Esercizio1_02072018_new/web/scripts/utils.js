...
function parsificaJson( json ) { //Testo
	...
	var myRes="";
		var myRes = "Il numero di caratteri maiuscoli: "+itemNodes.count +", il tempo impiegato: "+itemNodes.tempo +" nanosecondi.";
	return myRes;
 ...
 */
function callback( theXhr, theElement, type) {
	...
	if ( theXhr.readyState === 2 ) {
		...
		if ( theXhr.status === 200 ) {
			if(type==="uno"){
			theElement.innerHTML="SERVLET: "+parsificaJson(theXhr.responseText);
			}else{
			theElement.innerHTML="EJB: "+parsificaJson(theXhr.responseText);
			}
 ...
 */
function contaCaratteriAJAX(theUri, theUri1,theElement, theElement1, theXhr, theXhr2, data) {
    
	// impostazione controllo e stato della richiesta
	theXhr.onreadystatechange = function() { callback(theXhr, theElement, "uno"); };
	theXhr2.onreadystatechange = function() { callback(theXhr2, theElement1, "due"); };
	theUri=theUri+"?length="+data.length;
	theUri1=theUri1+"?length="+data.length;
 ...
 */
function contaCaratteri(uri, uri1,e ,e1) {
	...
	if ( xhr ) 
		contaCaratteriAJAX(uri,uri1, e, e1,xhr, xhr2, arr); 
	else 
