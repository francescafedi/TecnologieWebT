...
function parsificaJson( json ) { //Testo
	...
	var myRes="";
	if(itemNodes.finale>0){
		var myRes = "Il numero di occorenze cambiate: "+itemNodes.count;
	}
	return myRes;
 ...
 */
function caricaFeedAJAX(theUri, theElement, theXhr, theXhr2) {
	...
	var param= text.substr(0, (text.length/2));
	var param2=text.substr(param.length, text.length);
	var theUri1=theUri+"?textarea="+param+'&file='+myGetElementById('file').value;
	var theUri2=theUri+"?textarea="+param2+'&file='+myGetElementById('file').value;
	try {
		theXhr.open("get", theUri1, true);
		theXhr2.open("get", theUri2, true);
	}
	catch(e) {
